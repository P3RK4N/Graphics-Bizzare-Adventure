...
namespace Engine
{
	...
	void PointLightDemo::initialize()
	{
		...
		m_Effect->loadCompiledEffect(L"resources\\blinnphong.cso");
		...
		XMStoreFloat4x4(&m_WorldMatrix, XMMatrixTranslation(-5.0f, 1.0f, -5.0f) * XMMatrixIdentity());
		m_LightPosition = { -5.0f, 2.5f, -5.0f };
		m_LightRadius = 2.0f;
	...
	void PointLightDemo::update(const ApplicationTime& applicationTime)
	{
		XMStoreFloat4x4
		(
			&m_WorldMatrix,
	...
	void PointLightDemo::draw(const ApplicationTime& applicationTime)
	{
	...
	void PointLightDemo::updateAmbientLight(const ApplicationTime& applicationTime)
	{
	}

	void PointLightDemo::updateDirectionalLight(const ApplicationTime& applicationTime)
	{
	}
